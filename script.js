...
class Factory {
  constructor() {
    ...
    this.customerBuild = (color, options) => {
      return `Building one ${this.color} ${this.trim} ${this.model} with the following options: ${options}`
    }
  }
}
...
class Truck extends Factory {
  constructor(model, color, trim, enginesize, hitch, bed, navigation) {
    super()
    this.model = model
    this.color = color
    this.trim = trim
...
const mazda3 = new Car("mazda3", 4, "red", "hybrid", "automatic", "touring", "base", "premium", "leather", true)
...
console.log(miataRf.massBuild(15000, "Red"));
